* --------------------------------------------------------------------------------------------------
* Fuzzy Merge: Security Name Cleaning, Bonds
*
...
* cross-linkage.
*
* This file parses and cleans the security name field for bond records and extracts relevant info.
* --------------------------------------------------------------------------------------------------
...
gen name_jurisdict_categ = trim(regexs(0)) if regexm(securityname_cln, "(^| )(REPUBLIC OF)($| )|(^| )REPUBLIC($| )")
replace name_jurisdict_categ = trim(regexs(0)) if regexm(securityname_cln, "(^| )(GOVERNMENT OF)($| )|(^| )GOVERNMENT($| )") & name_jurisdict_categ==""
replace name_jurisdict_categ = trim(regexs(0)) if regexm(securityname_cln, "(^| )(KINGDOM OF)($| )|(^| )KINGDOM($| )") & name_jurisdict_categ==""
replace name_jurisdict_categ = trim(regexs(0)) if regexm(securityname_cln, "(^| )(GOVT OF)($| )|(^| )GOVT($| )|(^| )GOV($| )") & name_jurisdict_categ==""
replace name_jurisdict_categ = trim(regexs(0)) if regexm(securityname_cln, "(^| )(REP OF)($| )|(^| )REP($| )|(^| )REPUBBLICA($| )") & name_jurisdict_categ==""
replace name_jurisdict_categ = trim(regexs(0)) if regexm(securityname_cln, "(^| )REPUBLIK($| )|(^| )(REPUBLIQUE DU)($| )|(^| )REPUBLIQUE($| )") & name_jurisdict_categ==""
replace name_jurisdict_categ = trim(regexs(0)) if regexm(securityname_cln, "(^| )(COMMONWEALTH OF)($| )|(^| )COMMONWEALTH($| )") & name_jurisdict_categ==""
replace name_jurisdict_categ = trim(regexs(0)) if regexm(securityname_cln, "(^| )(COMWLTH OF)($| )|(^| )COMWLTH($| )") & name_jurisdict_categ==""
replace name_jurisdict_categ = trim(regexs(0)) if regexm(securityname_cln, "(^| )TREASURY($| )|(^| )TREAS($| )|(^| )TSY($| )") & name_jurisdict_categ==""
replace name_jurisdict_categ = trim(regexs(0)) if regexm(securityname_cln, "(^| )MUNICIPAL($| )|(^| )MUNI($| )|(^| )MUN($| )") & name_jurisdict_categ==""
replace name_jurisdict_categ = trim(regexs(0)) if regexm(securityname_cln, "(^| )(STATE OF)($| )|(^| )STATE($)") & name_jurisdict_categ==""
replace name_jurisdict_categ = trim(regexs(1)) if regexm(securityname_cln, "((^| )STATE)( GO($| ))") & name_jurisdict_categ==""
replace name_jurisdict_categ = trim(regexs(3)) if regexm(securityname_cln, "((^| )GO )(STATE($| ))") & name_jurisdict_categ==""
...
replace name_bond_legal = trim(regexs(0)) if regexm(name_jurisdict_categ, "STATE|MUNI") & name_bond_legal=="" & regexm(securityname_cln, "(^| )GO($| )|(^| )REV($| )")

* strip the extracted words from clean name
replace securityname_cln = subinstr(securityname_cln, name_firm_categ, "", .)
replace securityname_cln = subinstr(securityname_cln, name_jurisdict_categ, "", .)
...
* collapse categories of bonds
replace name_jurisdict_categ = subinstr(name_jurisdict_categ, "OF", "", .)
replace name_jurisdict_categ = trim(name_jurisdict_categ)
replace name_jurisdict_categ = "COMWLTH" if regexm(name_jurisdict_categ, "COMMONWEALTH")
replace name_jurisdict_categ = "GOVT" if regexm(name_jurisdict_categ, "GOV|GOVERNMENT")
replace name_jurisdict_categ = "MUNI" if regexm(name_jurisdict_categ, "MUNICIPAL|MUN")
replace name_jurisdict_categ = "REP" if regexm(name_jurisdict_categ, "REPUBBLICA|REPUBLIC|REPUBLIK|REPUBLIQUE|REPUBLIQUE DU")
replace name_jurisdict_categ = "TREAS" if regexm(name_jurisdict_categ, "TREASURY|TSY")
